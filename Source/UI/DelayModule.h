...
public:
          ...
          linkOffset(0.0)
    {
        ...
        // Start animation timer for icon (30 FPS)
        startTimerHz(30);
    }
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        // Draw icon in icon area with animation phase
        auto iconBounds = getIconBounds().toFloat();
        ModuleIcons::drawIcon(g, ModuleIcons::IconType::Haas, iconBounds, getActive(), animationPhase);
    }
...
private:
    ...
    static constexpr float ANIMATION_SPEED = 0.5f;  // Complete cycle in ~2 seconds at 30 FPS
    ...
    {
        // Update animation phase (0.0 to 1.0, wrapping)
        animationPhase += ANIMATION_SPEED / 30.0f;  // Adjust for 30 FPS
        if (animationPhase >= 1.0f)
            animationPhase -= 1.0f;

        // Only repaint the icon area for efficiency
